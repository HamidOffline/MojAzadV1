...
object SettingsManager {
    ...
    private const val MOJAZAD_DOUBLE_COLUMN_DEFAULT_INITIALIZED =
        "mojazad_double_column_default_initialized"
     ...
     * it creates a new default subscription.
     */
    ...
    fun getSocksUsername(): String? {
        return MmkvManager.decodeSettingsString(AppConfig.PREF_SOCKS_USERNAME)
            ?.trim()
            ?.takeIf { it.isNotEmpty() }
    }

    fun getSocksPassword(): String? {
        return MmkvManager.decodeSettingsString(AppConfig.PREF_SOCKS_PASSWORD)
            ?.trim()
            ?.takeIf { it.isNotEmpty() }
    }
    ...
    private fun IsDynamicSocksPort(): Boolean {
        return MmkvManager.decodeSettingsBool(
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns =
            MmkvManager.decodeSettingsString(AppConfig.PREF_DOMESTIC_DNS)
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns =
            MmkvManager.decodeSettingsString(AppConfig.PREF_REMOTE_DNS)
                ?: AppConfig.DNS_PROXY
    ...
    fun getCurrentVpnInterfaceAddressConfig(): VpnInterfaceAddressConfig {
        ...
        return VpnInterfaceAddressConfig
            .getConfigByIndex(
                selectedIndex ?: 0
            )
    }
    ...
    /**
     * Check if a root run mode is selected.
     */
    ...
    private fun ensureDefaultSettings() {
        ...
        ensureDefaultValue(
            AppConfig.PREF_FRAGMENT_MAXSPLIT,
            "10"
        )
         ...
         * MojAzad default:
         *
         * Enable double-column server display by default.
         *
         * This is applied only once.
         * After that, the user can freely turn it on or off.
         */
        ensureMojAzadDoubleColumnDefault()
    }

    /**
     * Enable double-column display only once as MojAzad's default.
     *
     * We use a separate marker so that after initialization
     * the user's own preference is respected.
     */
    private fun ensureMojAzadDoubleColumnDefault() {
    ...
    private fun migrateHysteria2PinSHA256() {
        ...
        for (guid in serverList) {
            ...
            if (
                profile.pinSHA256.isNullOrEmpty() ||
    ...
    /**
     * Migrates server list from legacy storage
     * to subscription-based storage.
     */
    private fun migrateServerListToSubscriptions() {

        val migrationKey =
            "server_list_to_subscriptions_migrated"
        ...
        val oldJson =
            MmkvManager.readLegacyServerList()
