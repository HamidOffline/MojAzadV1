...
object AngConfigManager {
    ...
    fun share2Clipboard(
        ...
        try {
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to share config to clipboard",
                e
            )
    ...
    fun shareNonCustomConfigsToClipboard(
        ...
        try {
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to share non-custom configs to clipboard",
                e
            )
    ...
    fun share2QRCode(
        ...
        try {
            ...
            return QRCodeDecoder.createQRCode(
                conf
            )
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to share config as QR code",
                e
            )
    ...
    fun shareFullContent2Clipboard(
        context: Context,
        guid: String?
    ): Int {
        ...
        try {
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to share full content to clipboard",
                e
            )
        ...
        try {
            ...
            return config.configType.protocolScheme +
                when (config.configType) {
                    ...
                    EConfigType.VMESS ->
                        VmessFmt.toUri(config)

                    EConfigType.SHADOWSOCKS ->
                        ShadowsocksFmt.toUri(config)

                    EConfigType.SOCKS ->
                        SocksFmt.toUri(config)

                    EConfigType.VLESS ->
                        VlessFmt.toUri(config)

                    EConfigType.TROJAN ->
                        TrojanFmt.toUri(config)

                    EConfigType.WIREGUARD ->
                        WireguardFmt.toUri(config)

                    EConfigType.HYSTERIA2 ->
                        Hysteria2Fmt.toUri(config)
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to share config for GUID: $guid",
                e
            )
    ...
    private fun parseBatchSubscription(
        servers: String?
    ): Int {
        ...
        try {
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to parse batch subscription",
                e
            )
        }
    ...
    private fun batchSaveConfigs(
        ...
        val keyToProfile =
            mutableMapOf<String, ProfileItem>()
        ...
        configs.forEach { config ->
            ...
            MmkvManager.encodeProfileDirect(
                key,
                JsonUtil.toJson(
                    config
                )
    ...
    private fun findMatchedProfileKey(
        ...
        return keyToProfile.keys
            .firstOrNull()
    }

    private fun getRemovedSelectedProfile(
        subid: String,
        append: Boolean
    ): ProfileItem? {
    ...
    private fun parseCustomConfigServer(
        ...
        if (
            ...
            try {
                ...
                LogUtil.e(
                    AppConfig.TAG,
                    "Failed to parse custom config server JSON array",
                    e
                )
            }
            ...
            try {
                ...
                LogUtil.e(
                    AppConfig.TAG,
                    "Failed to parse custom config server as single config",
                    e
                )
            }
            ...
            server.startsWith("[Interface]") &&
            server.contains("[Peer]")
        ) {
    ...
    private fun parseConfig(
        ...
        try {
            ...
            if (
                subItem?.filter.isNotNullEmpty() &&
                config.remarks.isNotNullEmpty()
            ) {

                val matched =
                    Regex(
                        pattern =
                            subItem?.filter.orEmpty()
                    )
                        .containsMatchIn(
                            input =
                                config.remarks
    ...
    fun updateConfigViaSubAll():
        ...
        return try {
            ...
            LogUtil.e(
                AppConfig.TAG,
                "Failed to update config via all subscriptions",
                e
            )
    ...
    fun updateConfigViaSub(
        ...
        try {
            ...
            try {
                ...
                LogUtil.e(
                    AppConfig.TAG,
                    "Subscription proxy request failed",
                    e
                )
            }
            ...
            /*
             * Fallback:
             * direct connection without local proxy.
             */
            if (configText.isBlank()) {
                ...
                try {
                    ...
                    LogUtil.e(
                        AppConfig.TAG,
                        "Subscription direct request failed",
                        e
                    )
                }
            }
            ...
            /*
             * Last fallback:
             * preserve original v2rayNG behavior.
             */
            if (configText.isBlank()) {
                ...
                try {
                    ...
                    LogUtil.e(
                        AppConfig.TAG,
                        "Subscription legacy request failed",
                        e
                    )
                }
            }
            ...
            if (
                ...
                LogUtil.i(
                    AppConfig.TAG,
                    "Subscription usage: " +
                        "upload=${it.subscription.uploadBytes}, " +
                        "download=${it.subscription.downloadBytes}, " +
                        "total=${it.subscription.totalBytes}, " +
                        "expire=${it.subscription.expireTime}"
                )
                ...
                LogUtil.i(
                    AppConfig.TAG,
                    "subscription-userinfo header not provided"
                )
            }
            ...
            if (count <= 0) {
                ...
                LogUtil.w(
                    AppConfig.TAG,
                    "Subscription downloaded but no valid configs were parsed"
                )
    ...
    private fun importUrlAsSubscription(
        url: String
    ): Int {
